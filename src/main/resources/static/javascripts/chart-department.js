...
//populate years
		const yearForCounts=document.getElementById('Année');
        const currentYear = new Date().getFullYear();
        const minYearInput = document.getElementById('Annéemin');
...
function updateBarChart1() {
    ...
    fetch(`/department/vacancies-by-time/${selectedYear}/${selectedMonth}`)
        ...
        .then(data => {
            ...
            let chartTitle = `Nombre de postes vacants pour "${departmentName}"`;
            if (selectedMonth !== '0') {
