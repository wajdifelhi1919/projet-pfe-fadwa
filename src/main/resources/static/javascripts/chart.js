    ...
    fetch(`/hr/vacancies-filter-by-department-time/${selectedDepartment}/${selectedYear}/${selectedMonth}`)
        ...
        .then(data => {
            ...
            if(selectedDepartment === '0' && selectedMonth === '0' && selectedYear === '0'){
				chartTitle += 'Nombre de postes vacants';
			}
            if (selectedDepartment !== '0') {
                chartTitle += `Nombre de postes vacants pour le départementt "${departmentName}"`;
            ...
            if (selectedYear !== '0') {
                chartTitle += ` in ${year}`;
				   ...
				   if (selectedMonth === '0' && selectedYear !== '0') {
               		 chartTitle += `Nombre de postes vacants en ${year}`;
            }
            	 else if (selectedMonth !== '0' && selectedYear !== '0'){
					 chartTitle += `Nombre de postes vacants pour ${month} en ${year}`;
			  }
			}
    ...
    function updateBarChart2() {
        ...
        fetch(`/hr/vacancies-filter-by-position-time/${selectedPosition}/${selectedYear}/${selectedMonth}`)
            ...
            if (selectedPosition !== '0') {
                chartTitle += `Nombre de postes vacants pour la poste "${positionName}"`;
            }
            
              	if (selectedMonth !== '0') {
                chartTitle += ` pour ${month}`;
            }
...
function updateLineChart() {
    ...
    let chartTitle = 'Tendance de la demande de poste';
    if (selectedStartYear !== '0') {
        if (selectedEndYear !== '0') {
            chartTitle = `Tendance de la demande de posted de ${selectedStartYear} a ${selectedEndYear}`;
        } else {
...
function createBarChart3(data, departmentName, isUrgent) {
    ...
    if (departmentName) {
        const titlePrefix = isUrgent ? 'Nombre des candidats pour les postes vacants urgents' : 'Nombre des candidats pour les postes vacants actifs';
        chartTitle = `${titlePrefix} in "${departmentName}"`;
    } else {
        chartTitle = isUrgent ? 'Nombre des candidats pour les postes vacants urgents' : 'Nombre des candidats pour les postes vacants actifs';
    }
...
function createBarChart4(data) {
    ...
    function updateBarChart4() {
        ...
        fetch(`/hr/candidates-by-position/${selectedDepartment}/${selectedYear}/${selectedMonth}`)
            ...
            let chartTitle = '';
            if(selectedDepartment === '0' && selectedMonth === '0'  && selectedYear === '0'){
				chartTitle += 'Nombre des candidats\n';
			}
            
            if (selectedDepartment !== '0') {
                chartTitle += `Nombre des candidats pour le département "${departmentName}"`;
            ...
            if (selectedYear !== '0') {
                chartTitle += ` en ${year}`;
				   ...
				   if (selectedMonth === '0' && selectedYear !== '0') {
               		 chartTitle += `Nombre des candidats en ${year}`;
            }
            	 else if (selectedMonth !== '0' && selectedYear !== '0'){
					 chartTitle += `Nombre des candidats pour ${month} en ${year}`;
			  }
			}
...
function updateLineChart2() {
    ...
    const selectedEndYear = endYearSelect2.value || '0';
         ...
         // Update chart title based on selected years
    let chartTitle = 'Tendance des candidats\n';
    if (selectedStartYear !== '0') {
        if (selectedEndYear !== '0') {
            chartTitle = `Tendance des candidats de ${selectedStartYear} to ${selectedEndYear}`;
        } else {
            chartTitle = `Candidats en ${selectedStartYear}`;
        }
    }
...
function updateVacancyOptions() {
    ...
    if (selectedPositionId === '0') {
        ...
        fetch(`/hr/vacancies-by-position/${selectedPositionId}`)
            ...
            .catch(error => console.error('Erreur lors de la récupération des postes vacants\n:', error));
    }
}
...
function createDoughnutChart(data, vacancyName) {
    ...
    if (positionSelectOption2.value === '0' && vacancySelectOption.value === '0') {
        title = 'Pourcentages de candidats par statut pour tous les postes vacants';
    } else {
